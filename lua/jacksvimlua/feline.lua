...
vim.cmd
[[
	hi clear FelineViMode
	augroup file_color
		autocmd!
		autocmd BufReadPre,BufNewFile,FileType,VimEnter,WinClosed * lua local icon, color = require('nvim-web-devicons').get_icon(vim.fn.expand '%:t', vim.fn.expand '%:e', {default = true}); vim.b.file_icon, vim.b.file_color = icon, string.format('#%06x', vim.api.nvim_get_hl_by_name(color, true).foreground)
	augroup end
]]
	...
	{ -- {{{
		active =
		{
			{ -- Left {{{
				{
					...
					right_sep = function() return
						{
							hl = {fg = _SIDEBAR[1], bg = vim.b.file_color},
				...
				{
					hl = function() return {fg = _SIDEBAR[1], bg = vim.b.file_color} end,
					provider  = function() return ' '..vim.b.file_icon..' ' end,
					right_sep = function() return
						{
							hl = {fg = _SIDEBAR[1], bg = vim.b.file_color},
							str = _LEFT_SEPARATOR,
			...
			{ -- Right {{{
				...
				{
					hl = function() return {fg = _BLACK[1], bg = vim.b.file_color, style = 'bold'} end,
					left_sep = function() return
						{
							hl = {fg = vim.b.file_color, bg = _SIDEBAR[1]},
							str = _LEFT_SEPARATOR,
						}
					end,
					provider = 'file_type',
		...
		inactive =
		{
			...
			{{ -- Right {{{
				...
				provider = function(_, win_id) return _BUF_ICON[vim.bo[vim.api.nvim_win_get_buf(win_id)].filetype] or '' end,
			}}, -- }}}
		},
	}, -- }}}
